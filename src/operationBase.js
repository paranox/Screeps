...
OperationBase.prototype.update = function()
{
    ...
    if (this.home.spawnOrdersPlaced && this.home.spawn.memory.spawnQueue)
    {
        ...
        for (const id in this.home.spawnOrdersPlaced)
        {
            ...
            if (!spawnQueueEntry)
            {
            	console.log("Operation " + this.opName + "[" + this.id + "]: Couldn't find spawn order " + id);
                continue;
            }
            //else
            //	console.log("Operation " + this.opName + "[" + this.id + "]: Checking spawn order " + id);
